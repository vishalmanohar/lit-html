...
export const compileLitTemplates = (): ts.TransformerFactory<ts.SourceFile> => {
  ...
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    ...
    const expressionToTemplate = new Map<ts.TaggedTemplateExpression, TemplateInfo>();
    ...
    const rewriteTemplates = (node: ts.Node): ts.VisitResult<ts.Node> => {
      ...
      if (topLevelStatementToTemplate.has(node as ts.Statement)) {
        ...
        const stringsArrayExpression = f.createArrayLiteralExpression(
          ts.isNoSubstitutionTemplateLiteral(templateExpression)
            ...
            : [
                ...
                ...templateExpression.templateSpans.map((s) => f.createStringLiteral(s.literal.text))]
        );
        ...
        const parts: Array<
          ...
          {type: PartType, index: number, name: string, strings: Array<string>, tagName: string}
        > = [];
        const [html, attrNames] = _getTemplateHtml(
          ts.isNoSubstitutionTemplateLiteral(templateExpression)
            ? [templateExpression.text] as any as TemplateStringsArray
            : [
        ...
        const partsArrayExpression = f.createArrayLiteralExpression(
          parts.map((part) => {
            const partProperties = [
              f.createPropertyAssignment('_type', f.createNumericLiteral(part.type)),
            ...
            if (part.type === PartType.ATTRIBUTE ||
              ...
              partProperties.push(
                f.createPropertyAssignment('name', f.createNumericLiteral(part.name)),
                f.createPropertyAssignment('_strings', f.createArrayLiteralExpression(
                  part.strings.map((s) => f.createStringLiteral(s))
                )),
                f.createPropertyAssignment('_constructor', 
            ...
            [
              f.createVariableDeclaration(
                ...
                f.createObjectLiteralExpression([
                  f.createPropertyAssignment('_strings', stringsArrayExpression),
      ...
      if (isLitTemplate(node)) {
        ...
        return f.createObjectLiteralExpression([
          ...
          f.createPropertyAssignment('values',
