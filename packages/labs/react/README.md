...
`useController` uses `useState` to create and store an instance of a controller and a `ReactControllerHost`. If then calls the controllers lifecycle from the hook body and `useLayoutEffect` callbacks, emulating the `ReactiveElement` lifecycle as closely as possible. `ReactControllerHost` implements `addController` so that controller composition works and nested controller lifecycles are called correctly. `ReactControllerHost` also implements `requestUpdate` by calling a `useState` setter, so that a controller with new renderable state can cause its host component to re-render.
...
| connected      | useState initial value              |
| disconnected   | useLayoutEffect cleanup, empty deps |
| willUpdate     | hook body                           |
| update         | hook body                           |
| updated        | useLayoutEffect cleanup, no deps    |
| requestUpdate  | useState setter                     |
| updateComplete | useLayoutEffect cleanup, no deps    |
